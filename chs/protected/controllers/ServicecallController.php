...
/**
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->job_status_id < 100) /*THIS LOCKsJOBS with status greater than 100 to edit */ {
    ...
    public function actionSubmitclaimtocontractor()
    {
        ...
        if (isset($_POST['Workcarriedout'])) {
            ...
            if ($workcarriedoutmodel->validate()) {
                ...
                if ($workcarriedoutmodel->checkiffileuploaded($workcarriedoutmodel->product_plating_image)) {
                ...
                $wco_array['first_visit_date'] = date('d-M-Y',$workcarriedoutmodel->first_visit_date);
                $wco_array['job_completion_date'] = date('d-M-Y',$workcarriedoutmodel->job_completion_date);
                ...
                $model->remote_data_sent = json_encode($wco_array);
                ...
                $model->job_status_id = '35';///it means job is submitted



                $post_data_array=array();

                $post_data_array['claim_content']=$wco_array;
                $post_data_array['communications']=$full_chat_array;
                $post_data_array['status_log']=json_decode($model->activity_log,true);


                $data_type='claim_submission_servicecall_data';

                $return_data_array['data_type'] = $data_type;///is it a message or  servicecall_data

                $return_data_array['engineer_email'] = $model->contract->portal_login_email;

                $return_data_array['service_reference_number'] = $model->remote_ref_no;

                $return_data_array['api_key'] =$model->contract->api_key;


                $return_data_array['sent_data'] = $post_data_array;



                $data_to_be_sent_string = json_encode($return_data_array);

                $model->remote_data_sent =   $data_to_be_sent_string ;



                if ($model->save()) {

                    $system_message.= '<div class="flash-success">Claim  successfully Submitted</div>';

                    //$system_message.= Servicecall::model()->sendclaimviaapi($model->id);
        ...
        echo $system_message;
        $this->render('workcarriedout', array('workcarriedoutmodel'=>$workcarriedoutmodel, 'model'=>$model, ));
